-- Register Earn-screen tasks that were missing from the points catalog so that
-- award_task_points() no longer returns 'unknown_task' for them:
--   * monthly-expenses-update (Monthly Expenses Update, 500, resets monthly)
--   * book-life-counseling     (Book Life Coaching Session, 500, resets monthly)
...
begin
  ...
  return case p_task_id
    ...
    when 'book-life-counseling' then 500
    ...
    when 'monthly-expenses-update' then 500
...
begin
  case p_task_id
    ...
    when 'book-life-counseling' then
      return 'book-life-counseling-' || v_month_str;
    ...
    when 'monthly-expenses-update' then
      return 'monthly-expenses-update-' || v_month_str;
...
begin
  ...
  if v_points <= 0 and p_task_id not in (
    ...
    'counselling-session', 'counselling-reflection', 'book-life-counseling',
    'plan-line-item', 'plan-section-complete', 'full-plan-complete',
    'monthly-budget-update', 'monthly-expenses-update',
