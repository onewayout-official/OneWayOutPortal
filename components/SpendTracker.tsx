...
function fallbackStatusFromMetadata(
  ...
  const { statusLabel, isActive } = giftcardStatusFromState(stateId);
  const issuedCents =
    amountRand != null && Number.isFinite(amountRand) ? Math.round(amountRand * 100) : 0;
  return {
    id: giftcardId,
    stateId,
    statusLabel,
    isActive,
    remainingCents: isActive ? issuedCents : 0,
    issuedAmount: issuedCents || undefined,
    isPartiallyRedeemed: false,
  };
}
  ...
  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === "visible") {
        ...
        void loadGiftcardStatuses(spendingHistory);
      }
    };
    document.addEventListener("visibilitychange", handleVisibility);
    return () => document.removeEventListener("visibilitychange", handleVisibility);
  }, [refreshPointsBalance, loadGiftcardStatuses, spendingHistory]);
        ...
        {spendingHistory.length > 0 ? (
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {spendingHistory.map((transaction) => {
              ...
              const issuedCents =
                status?.issuedAmount ??
                (amountRand != null ? Math.round(amountRand * 100) : null);
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {status && status.statusLabel !== "Unknown" && (
                        ...
                        >
                          {giftcardStatusBadgeText(status)}
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      ...
                      {amountRand != null ? ` · R ${amountRand.toFixed(2)} issued` : ""}
                    </p>
                    {showBalanceDetail && issuedCents != null && (
                      <p className="mt-1 text-xs text-gray-600 dark:text-gray-400">
                        Issued R {formatCentsAsRand(issuedCents)}
                        {status.isActive
                          ? ` · R ${formatCentsAsRand(status.remainingCents)} left`
                          : status.statusLabel === "Redeemed"
                            ? " · fully used"
                            : ""}
                      </p>
                    )}
                    {expiryDate && status?.isActive && (
