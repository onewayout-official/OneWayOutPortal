...
function slotStatusClass(status: AvailabilitySlot["status"]): string {
  if (status === "available") {
...
function slotStatusLabel(status: AvailabilitySlot["status"]): string {
  if (status === "available") return "Book session";
...
export default function BookFinancialSessionPage() {
  ...
  const fetchRange = useMemo(() => {
    const from = today;
    const to = toISODate(new Date(Date.now() + 42 * 24 * 60 * 60 * 1000));
    return { from, to };
  }, [today]);
  ...
  const loadAvailability = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/financial-session/availability?${params.toString()}`,
        { method: "GET", headers }
      );
      ...
      setGraphSyncStatus(json.graphSyncStatus ?? "error");
    } catch (err) {
    ...
    () =>
      parseAvailabilityList(availability)
        .map((slot) => {
          if (slot.kind === "date") {
            const status =
              slotByKey.get(`${slot.date}|${slot.time}`)?.status ?? "outside_hours";
            return {
              key: slot.raw,
              dayLabel: slot.date,
              time: slot.time,
          ...
          const status =
            slotByKey.get(`${nextDate}|${slot.time}`)?.status ?? "outside_hours";
          return {
            key: slot.raw,
  ...
  const openBookingConfirm = (date: string, time: string) => {
    ...
    const slot = slotByKey.get(`${date}|${time}`);
  ...
  const confirmBooking = async () => {
    ...
    try {
      const headers = await getAuthHeader();
      const response = await fetch("/api/financial-session/book", {
        method: "POST",
        headers,
        body: JSON.stringify({
      ...
      const json = (await response.json()) as {
        appointment?: { meetingLink?: string };
        error?: string;
      };
      if (!response.ok) {
  ...
  const selectedSlotAvailable =
    selectedDate &&
    selectedTime &&
    slotByKey.get(`${selectedDate}|${selectedTime}`)?.status === "available";
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              Choose an available 20-minute time slot below. Open slots reflect the coach&apos;s
              portal schedule and Outlook calendar when synced.
            </p>
          </div>
          ...
          <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:col-span-1">
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Recurring weekly slots from the financial coach schedule.
              </p>
                ...
                <div className="mt-2 space-y-2">
                  {todaySlots.length > 0 ? (
                    todaySlots.map((slot) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className={`mt-2 w-full rounded-md px-3 py-1.5 text-xs font-medium disabled:cursor-not-allowed disabled:opacity-60 ${slotStatusClass(
                            slot.status
                          )}`}
                        >
                    ...
                    <p className="rounded-lg border border-dashed border-gray-300 px-3 py-2 text-xs text-gray-500 dark:border-gray-600 dark:text-gray-400">
                      {isLoading ? "Loading slots…" : "No slots scheduled for today."}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="mt-5">
                ...
                <div className="mt-2 space-y-2">
                  {weeklySlots.length > 0 ? (
                    ...
                    <p className="rounded-lg border border-dashed border-gray-300 px-3 py-2 text-xs text-gray-500 dark:border-gray-600 dark:text-gray-400">
                      {isLoading ? "Loading…" : "No weekly slots configured yet."}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
              <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-5 shadow-xl dark:border-gray-700 dark:bg-gray-800">
                ...
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                  Book a 20-minute financial planning session on {pendingBooking.date} at{" "}
                  {pendingBooking.time}?
                </p>
                {bookingError && (
              ...
              <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-5 shadow-xl dark:border-gray-700 dark:bg-gray-800">
                ...
                {bookingPopup.meetingLink ? (
                  ...
                  <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
                    ...
                    <p className="mt-1">
                      Your meeting link will appear here once Microsoft Teams is configured for
                      this coach.
                    </p>
                  </div>
                )}
